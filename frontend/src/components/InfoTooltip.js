    ...
    return(
        <section className={`popup popup_type_infotooltip ${props.isOpen ? 'popup_opened' : ''}`} onClick={props.onCloseByOverlay}>
            <div className='popup__container popup__container_type_infotooltip'>
                ...
                <div className="popup__image" style={{backgroundImage: `url(${props.signedUp ? approveSign : redCross})`}} />
                <h2 className="popup__title popup__title_type_infotooltip">{props.signedUp ? 'Вы успешно зарегистрировались!' : 'Что-то пошло не так! Попробуйте ещё раз.'}</h2>
            </div>
        </section>
    )
}
